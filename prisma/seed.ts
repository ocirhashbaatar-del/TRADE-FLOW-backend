...
async function main() {
  ...
  const admin = await prisma.user.upsert({ where: { email: 'ocirhashbaatar@gmail.com' }, update: { tenantId: tenant.id, passwordHash: adminPasswordHash, role: Role.ADMIN, platformAdmin: true }, create: { name: 'TradeFlow Admin', email: 'ocirhashbaatar@gmail.com', passwordHash: adminPasswordHash, role: Role.ADMIN, platformAdmin: true, tenant: 'TradeFlow', tenantId: tenant.id } })
  const transporter = await prisma.user.upsert({ where: { email: 'gardi@gmail.com' }, update: { tenantId: tenant.id, passwordHash: transporterPasswordHash, role: Role.TRANSPORTER }, create: { name: 'TradeFlow Тээвэрлэгч', email: 'gardi@gmail.com', passwordHash: transporterPasswordHash, role: Role.TRANSPORTER, tenant: 'TradeFlow', tenantId: tenant.id } })
