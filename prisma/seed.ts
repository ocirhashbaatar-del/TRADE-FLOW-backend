...
async function main() {
  ...
  const admin = await prisma.user.upsert({ where: { email: 'ocirhashbaatar@gmail.com' }, update: { passwordHash: adminPasswordHash, emailVerified: new Date(), tenant: tenant.name, tenantId: tenant.id, role: Role.ADMIN, platformAdmin: true }, create: { name: 'TradeFlow Admin', email: 'ocirhashbaatar@gmail.com', passwordHash: adminPasswordHash, emailVerified: new Date(), role: Role.ADMIN, platformAdmin: true, tenant: tenant.name, tenantId: tenant.id } })
