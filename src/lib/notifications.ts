...
export async function sendNotification(input:{userId:string;type:NotificationType;title:string;description:string;idempotencyKey:string}){const preference=await prisma.notificationPreference.findUnique({where:{userId_type:{userId:input.userId,type:input.type}}}),user=await prisma.user.findUnique({where:{id:input.userId},select:{email:true}});let row:NotificationRow|null=null;if(preference?.inApp??true){const created=await prisma.$queryRaw<NotificationRow[]>`
  INSERT INTO "Notification" ("userId", title, description, type, read, "createdAt")
  VALUES (${input.userId}, ${input.title}, ${input.description}, ${input.type}, false, NOW())
