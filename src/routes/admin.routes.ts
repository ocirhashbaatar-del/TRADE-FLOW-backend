...
const router = Router()
router.use(authenticate, authorizeEmails('ocirhashbaatar@gmail.com'))
...
router.get('/permissions', async (req, res) => res.json(await prisma.rolePermission.findMany({ where: { tenantId: tenantId(req) }, orderBy: [{ role: 'asc' }, { module: 'asc' }] })))
router.put('/permissions', async (req, res) => { const input = z.object({ role: z.nativeEnum(Role), module: z.string().min(2), canRead: z.boolean(), canCreate: z.boolean(), canUpdate: z.boolean(), canDelete: z.boolean() }).parse(req.body); const row = await prisma.rolePermission.upsert({ where: { tenantId_role_module: { tenantId: tenantId(req), role: input.role, module: input.module } }, update: input, create: { ...input, tenantId: tenantId(req) } }); await audit(req, 'UPSERT', 'RolePermission', row.id, undefined, row); res.json(row) })
