...
router.get('/saved-orders', async (req, res) => { const customer = await prisma.customerAccount.findFirst({ where: { tenantId: tid(req), userId: req.user!.id, active: true } }); if (!customer) return res.status(403).json({ message: 'B2B эрхгүй.' }); res.json(await prisma.savedOrderTemplate.findMany({ where: { tenantId: tid(req), customerId: customer.id }, orderBy: { updatedAt: 'desc' } })) })
router.post('/saved-orders', async (req, res) => { const customer = await prisma.customerAccount.findFirst({ where: { tenantId: tid(req), userId: req.user!.id, active: true } }); if (!customer) return res.status(403).json({ message: 'B2B эрхгүй.' }); const input = z.object({ name: z.string().min(2), lines: z.array(z.object({ productId: z.string(), variantId: z.string().optional(), quantity: z.number().int().positive() })).min(1) }).parse(req.body); const row = await prisma.savedOrderTemplate.upsert({ where: { tenantId_customerId_name: { tenantId: tid(req), customerId: customer.id, name: input.name } }, update: { lines: input.lines }, create: { tenantId: tid(req), customerId: customer.id, name: input.name, lines: input.lines, createdBy: req.user!.id } }); res.status(201).json(row) })
router.delete('/saved-orders/:id', async (req, res) => { const customer = await prisma.customerAccount.findFirst({ where: { tenantId: tid(req), userId: req.user!.id, active: true } }); if (!customer) return res.status(403).json({ message: 'B2B эрхгүй.' }); await prisma.savedOrderTemplate.deleteMany({ where: { id: String(req.params.id), tenantId: tid(req), customerId: customer.id } }); res.status(204).send() })
