...
const availableStock = async (productId: string, fallback: number) => {
  const balances = await prisma.inventoryBalance.aggregate({ where: { productId }, _sum: { onHand: true, reserved: true } })
...
router.get('/', async (req, res) => {
  ...
  res.json({ cart: cart.map((item) => ({ ...productShape(item.product), qty: item.quantity })), savedProductIds: saved.map((item) => item.productId) })
})

router.post('/cart/:productId', async (req, res) => {
  const { quantity } = z.object({ quantity: z.coerce.number().int().min(1).max(99).default(1) }).parse(req.body)
  ...
  const available = await availableStock(productId, product.stock)
...
router.patch('/cart/:productId', async (req, res) => {
  ...
  const result = await prisma.cartItem.updateMany({ where: { userId: req.user!.id, productId }, data: { quantity } })
  result.count ? res.json({ quantity }) : res.status(404).json({ message: 'Сагсны бараа олдсонгүй.' })
})

router.delete('/cart/:productId', async (req, res) => {
  await prisma.cartItem.deleteMany({ where: { userId: req.user!.id, productId: String(req.params.productId) } })
  res.status(204).send()
})
